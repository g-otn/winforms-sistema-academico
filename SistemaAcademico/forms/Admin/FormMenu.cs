...
namespace SistemaAcademico.forms.Admin
{
    public partial class FormMenu : Form
    {
        ...
        {
            colorirBotoes(btnVisaoGeral);
            Text = "Visão Geral - Sistema Acadêmico";
            if (formEstaAberto("FormVisaoGeral")) return;

            // Iniciar FormVisaoGeral
            FormVisaoGeral formvisao = new FormVisaoGeral();
            formvisao.MdiParent = this;
            formvisao.Dock = DockStyle.Fill;
            formvisao.Show();
        }

        private void btnUsuarios_Click(object sender, EventArgs e)
        {
            colorirBotoes(btnUsuarios);
            Text = "Usuários - Sistema Acadêmico";
            if (formEstaAberto("FormUsuarios")) return;

            // Iniciar FormUsuarios (Criação, pesquisa, alteração e exclusão de Usuários)            
        ...
        {
            colorirBotoes(btnMaterias);
            Text = "Matérias - Sistema Acadêmico";
            if (formEstaAberto("FormMaterias")) return;

            // Iniciar FormMaterias
